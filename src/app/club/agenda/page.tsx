...
import { 
  ...
  Download, FileText, Home, ArrowLeft, Shirt, Trophy
} from 'lucide-react';
...
export default function AgendaClub() {
  ...
  async function inicializar() {
    try {
      ...
      const { data: teamData } = await supabase.from('teams').select('*').eq('id', perfil.team_id).single();
      ...
      const { data: partidos } = await supabase
        ...
        .select(`
          *,
          ...
          home_team:teams!home_team_id(id, name, logo_url, main_shirt_color),
          away_team:teams!away_team_id(id, name, logo_url, main_shirt_color)
        `)
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center">Cargando Agenda...</div>;
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans pb-20">
      ...
      <div className="bg-white border-b px-6 py-4 flex items-center gap-3 sticky top-0 z-20">
        ...
        <h1 className="font-black text-xl text-slate-800">Mi Agenda</h1>
      </div>
      ...
      <div className="p-4 md:p-6 max-w-4xl mx-auto space-y-8">
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
            <button 
                ...
                className={`px-5 py-2 rounded-full font-bold text-sm whitespace-nowrap transition ${selectedCategory === 'Todas' ? 'bg-blue-600 text-white shadow-lg shadow-blue-200' : 'bg-white text-slate-500 border border-slate-200'}`}
            >
                Todas
            </button>
            {categories.map(cat => (
                <button 
                    ...
                    className={`px-5 py-2 rounded-full font-bold text-sm whitespace-nowrap transition ${selectedCategory === cat.name ? 'bg-blue-600 text-white shadow-lg shadow-blue-200' : 'bg-white text-slate-500 border border-slate-200'}`}
                >
                    {cat.name}
                </button>
            ))}
