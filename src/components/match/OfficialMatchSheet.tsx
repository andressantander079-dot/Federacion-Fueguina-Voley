...
export default function OfficialMatchSheet({ redirectAfterSubmit, readOnly = false }: OfficialMatchSheetProps) {
    ...
    const {
        ...
        setAllState // Important for hydration
    } = useVolleyMatch();
    ...
    const handleStatusChange = async (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        if (newStatus === 'live') {
            const confirmed = window.confirm("¡ATENCIÓN!\n\n¿Este partido está por comenzar?\n\nAl confirmar, el partido aparecerá EN VIVO para todo el público.");
            if (confirmed) {
                setMatchStatus('live');
                ...
                if (error) {
                    ...
                    setMatchStatus('scheduled'); // Revert UI
                }
            }
    ...
    useEffect(() => {
        ...
        const timeoutId = setTimeout(async () => {
            ...
            console.log("Saving sets to DB:", JSON.stringify(sets)); // DEBUG TRACE

            const { error } = await supabase
                .from('matches')
                .update({
                    sheet_data: currentSheetData
                    // Optimization columns temporarily removed to prevent schema errors
                    // home_score: sets[currentSetIdx].home,
                    // away_score: sets[currentSetIdx].away,
                    // current_set: sets[currentSetIdx].number
                })
                .eq('id', matchId);

            if (error) {
                console.error("Auto-save detailed error:", JSON.stringify(error, null, 2));
                setSaveStatus('error');
    ...
    }, [sets, currentSetIdx, posHome, posAway, benchHome, benchAway, staff, signatures, observations, servingTeam, readOnly, matchId, teamsInfo, supabase]); // Added supabase
                ...
                <div className="flex items-center gap-6">
                    ...
                    <div className="text-[10px] font-bold text-slate-400 text-right hidden md:block">
                        {readOnly ? (
                            ...
                            <>
                                {saveStatus === 'saving' && <span className="text-amber-500 animate-pulse">Guardando...</span>}
                                {saveStatus === 'saved' && <span className="text-green-500">Guardado ✔</span>}
